...
const createTree = ([rootEl,confirmDialog, errorDialog]) => {
...
const refreshTree = (extPanelWindow,sendResponse) => {
    ...
    browser.tabs.sendMessage(
        browser.devtools.inspectedWindow.tabId,
        { message: 'getDirectoryEntries' },
        (response) => {
            // Naive check to avoid unnecessary DOM updates.
            const newLength = response.directory && JSON.stringify(response.directory).length;
            if (newLength && main.dataset.lastLength !== newLength) {
                main.dataset.lastLength = newLength;
                
                if (Object.keys(response.directory).length === 0) {
                ...
                const entries = Object.entries(response.directory)
                .sort(([name], [nextName]) => {
...
const connectElement = (el) => {
  el.dataset.lastLength = 0;
  el.dataset.refresh = true;
  el.innerHTML = `<span class="spinner">⏳</span> Analyzing Origin Private File System`
  refreshTree(el);
};
...
browser.devtools.panels.create(
    ...
    (panel) => {
        ...
        panel.onShown.addListener((extPanelWindow) => {          
            ...
            connectElement(extPanelWindow.document.body.querySelector('main'));
            ...
            backgroundPageConnection.postMessage({
                name: 'init', tabId: browser.devtools.inspectedWindow.tabId,
            });
